      ...
      <html>
        ...
        <body>
          ...
          <div class="meta">
            ...
            <div><span>Contacto:</span> <span>${order.customer_phone || order.phone || '---'}</span></div>
    ...
    const content = `
      <div style="padding: 40px; font-family: 'Courier New', Courier, monospace; color: black; background: white; max-width: 500px; margin: auto;">
        ...
        <div style="margin-bottom: 25px; font-size: 12px; text-transform: uppercase; line-height: 1.6;">
          ...
          <div style="display: flex; justify-content: space-between;"><span>CONTACTO:</span> <span>${order.customer_phone || order.phone || '---'}</span></div>
                        ...
                        <div className="space-y-3 pt-4">
                           ...
                           <div className="flex space-x-2">
                               ...
                               <Button 
                                  ...
                                  className={`flex-1 py-3 text-[10px] space-x-2 border-2 ${['En Lista', 'Preparando', 'Listo', 'Confirmado'].includes(selectedOrder.status) ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-400 border-zinc-200 dark:border-zinc-700' : 'bg-primary text-white border-primary'}`}
                                  style={!['En Lista', 'Preparando', 'Listo', 'Confirmado'].includes(selectedOrder.status) ? { boxShadow: '4px 4px 0px 0px var(--primary-shadow-20)' } : undefined}
                                >
        ...
        {showInvoice && selectedOrder && (
          <div className="fixed inset-0 w-screen h-screen z-[1100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-xl">
             <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} className="bg-white text-zinc-900 w-[95%] max-w-sm p-6 md:p-8 font-mono relative shadow-2xl print-ticket max-h-[90vh] overflow-y-auto custom-scrollbar">
                {(() => {
                  const isPaid = selectedOrder.isPaid || selectedOrder.is_paid;
                  ...
                  const orderStartTime = selectedOrder.created_at 
                    ? new Date(selectedOrder.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }) 
                    : new Date().toLocaleTimeString();
                  const orderPaymentTime = isPaid 
                    ? (selectedOrder.updated_at ? new Date(selectedOrder.updated_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : new Date().toLocaleTimeString()) 
                    : 'PENDIENTE';
                  ...
                  return (
                    <>
                      ...
                      <div className="space-y-1 mb-8 text-[10px] uppercase">
                         ...
                         <div className="flex justify-between"><span>FECHA:</span><span>{new Date(selectedOrder.created_at || new Date()).toLocaleDateString()}</span></div>
                         ...
                         <div className="flex justify-between"><span>CLI:</span><span className="font-bold">{selectedOrder.customer_name}</span></div>
                         <div className="flex justify-between"><span>DOCUMENTO:</span><span className="font-bold">{selectedOrder.identification || '---'}</span></div>
                         <div className="flex justify-between"><span>CONTACTO:</span><span className="font-bold">{selectedOrder.customer_phone || selectedOrder.phone || '---'}</span></div>
                      ...
                      <div className="flex justify-between text-lg font-bold border-t-2 border-zinc-900 pt-2 mb-6"><span>TOTAL:</span><span>{selectedOrder.total}</span></div>
            ...
            <div className="w-full lg:w-1/3 pl-0 lg:pl-8 flex flex-col h-full mt-8 lg:mt-0 pt-8 lg:pt-0">
                ...
                <div className="border-t-4 border-primary pt-6 space-y-6">
                  ...
                  <Button 
                   ...
                   >
                     {editingOrderId ? 'Guardar Cambios' : 'Registrar Domicilio'}
                   </Button>
                </div>
            </div>
        </div>
