...
const ReportsManager = () => {
  ...
  if (loading) return (
    <div className="h-[60vh] flex flex-col items-center justify-center space-y-4">
       ...
       <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-text-dim">Procesando Inteligencia de Negocios...</p>
    </div>
  );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-l-8 border-primary pl-8">
        <div>
          ...
          <p className="text-text-dim tracking-[0.4em] text-xs uppercase mt-4 font-bold underline decoration-primary decoration-2 underline-offset-8">
            Reportes Operativos e Inteligencia Financiera
          </p>
        </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               <div className="bg-surface border-2 border-zinc-200 dark:border-zinc-900 p-8 relative overflow-hidden group">
                  ...
                  <p className="text-[10px] font-bold text-text-dim tracking-widest mb-2">// VENTAS ÚLTIMOS 30 DÍAS</p>
               ...
               <div className="bg-surface border-2 border-zinc-200 dark:border-zinc-900 p-8 relative overflow-hidden group">
                  ...
                  <p className="text-[10px] font-bold text-text-dim tracking-widest mb-2">// TOTAL PEDIDOS</p>
                  <h3 className="text-4xl font-serif text-text-bright">{stats?.total_orders_30d || 0}</h3>
                  <p className="text-[10px] font-bold text-primary mt-6 tracking-widest uppercase">Operaciones Completadas</p>
               </div>
               ...
               <div className="bg-surface border-2 border-zinc-200 dark:border-zinc-900 p-8 relative overflow-hidden group">
                  ...
                  <p className="text-[10px] font-bold text-text-dim tracking-widest mb-2">// TICKET PROMEDIO</p>
                  <h3 className="text-4xl font-serif text-text-bright">
                    {formatCurrency((stats?.total_revenue_30d || 0) / (stats?.total_orders_30d || 1))}
                  </h3>
                  <p className="text-[10px] font-bold text-text-dim mt-6 tracking-widest uppercase">Gasto por Comensal</p>
               </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
               <div className="lg:col-span-2 bg-surface border-2 border-zinc-200 dark:border-zinc-900 p-8">
                  <div className="flex justify-between items-center mb-10">
                     <h2 className="text-xl font-serif tracking-widest uppercase">Flujo de <span className="text-primary">Ventas</span></h2>
               ...
               <div className="bg-surface border-2 border-zinc-200 dark:border-zinc-900 p-8">
                  <h2 className="text-xl font-serif tracking-widest uppercase mb-10">Ranking <span className="text-primary">Productos</span></h2>
                  <div className="space-y-6">
                     {stats?.top_products?.map((item, i) => (
                        <div key={i} className="space-y-2">
                           <div className="flex justify-between items-end">
                              ...
                              <span className="text-[10px] font-mono text-primary font-bold">{item.total_qty} Uds</span>
                           </div>
                     ...
                     {(!stats?.top_products || stats.top_products.length === 0) && (
                        <div className="h-48 flex flex-col items-center justify-center opacity-30">
                           <AlertCircle size={32} />
                           <p className="text-[10px] font-bold mt-4">Sin datos de venta</p>
                        </div>
                     )}
                  </div>
               </div>
          ...
          >
            ...
            <div className="lg:col-span-1 space-y-6">
               ...
               >
                  ...
                  <div className="flex items-center space-x-2 text-[10px] font-bold uppercase tracking-widest pt-4 border-t border-white/20">
                     <Clock size={14} />
                     <span>Corte: {new Date().toLocaleTimeString()}</span>
                  </div>
               </div>

               <div className="bg-surface border-2 border-zinc-200 dark:border-zinc-900 p-8 space-y-6">
                  <h4 className="text-xs font-bold tracking-[0.2em] border-b-2 border-primary/20 pb-4 uppercase">Desglose por Método</h4>
                  <div className="space-y-4">
                     {[
                       ...
                       { label: 'No Especificado', key: 'Unknown', icon: <AlertCircle size={16} /> }
                     ].map(method => (
            ...
            <div className="lg:col-span-2 bg-surface border-2 border-zinc-200 dark:border-zinc-900 flex flex-col overflow-hidden shadow-xl">
               <div className="p-6 border-b-2 border-zinc-200 dark:border-zinc-900 flex justify-between items-center bg-black/5 dark:bg-white/5">
                  <h4 className="text-xs font-bold tracking-[0.2em] uppercase">Transacciones del Turno</h4>
                  <span className="text-[10px] font-mono bg-primary text-white px-3 py-1">{cashClosing?.count_day || 0} Operaciones</span>
               </div>
                     ...
                     <tbody className="text-[10px] font-bold uppercase">
                        {cashClosing?.orders?.map(order => (
                           <tr key={order.id} className="border-b border-zinc-100 dark:border-zinc-900 hover:bg-primary/5 transition-all group">
                              ...
                              <td className="p-4 text-text-bright truncate max-w-[150px]">{order.customer_name || 'MOSTRADOR'}</td>
                              <td className="p-4">
                                 <span className="px-2 py-0.5 bg-zinc-100 dark:bg-white/5 text-[8px] font-bold tracking-widest">
                                    {order.payment_method || 'SIN REG'}
                                 </span>
                              </td>
                        ...
                        {(!cashClosing?.orders || cashClosing.orders.length === 0) && (
                           <tr>
                              <td colSpan="4" className="p-20 text-center opacity-30 flex flex-col items-center">
                                 ...
                                 <p className="mt-4 tracking-widest">No hay ventas registradas hoy</p>
                              </td>
                           </tr>
                        )}
                     </tbody>
